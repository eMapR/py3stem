...
def extract_from_shp(lyr, df_xy, id_field=None, n_jobs=0):
    ...
    if n_jobs:
        ...
        for i, feature in enumerate(lyr):
            ...
            geom_coords = stem3.get_coords_from_geometry(geometry, multipart='split')
        ...
        for feature in lyr:
            ...
            #geom_coords = stem3.get_coords_from_geometry(geometry)
            min_x, max_x, min_y, max_y = geometry.GetEnvelope()
...
def main(params, out_dir=None, xy_txt=None, kernel=False, resolution=30, tile_id_field=None, n_jobs=0, band_start_year=1986):  # changed from 1984 to 1986 Peter Clary 3/28/2019
    ...
    if np.any(df_vars.by_tile):
        ...
        if '3933' in point_dict:    # Added if statement the reason for this can be found in Peter's jounal stem3 CONUS 2018 CC in KennedyLabCEOAS googel drive
            del point_dict['3933']
        #if len(point_dict) > 0:
...
#params = '/vol/v2/stem3/param_files/extract_xy_by_tsa_params_topovars.txt'
