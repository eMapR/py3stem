...
Options:
    ...
    --proj=<str>            OGR-interpretable projection string [default: EPSG:3338]
    ...
    --downloadLog=<str>     CSV file containing information about a current LT GEE run. If either a path or 'True' specified, the script will wait for tasks to finish downloading while lt_gee_processing.listenAndDownload() is running. This file is automatically created by . Either a path to a log file or
...
import lt_gee_processing3 as ltee



def main(chunkDir, outDir, clipFile, searchStr='', njobs=1, tileIdField='name', proj='EPSG:3338', returnOutDirs=False, overwrite=False, downloadLog=None, maxWait=14400):
...
if __name__ == '__main__':
    ...
    cl_args = {k: v for k, v in docopt.docopt(__doc__).items() if v is not None}

    # get rid of extra characters from doc string and 'help' entry
    args = {re.sub('[<>-]*', '', k): v for k, v in cl_args.items()
            if k != '--help' and k != '-h'}  
