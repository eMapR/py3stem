...
from shp_functions3 import attributes_to_df
...
OAUTH_DIR = '/vol/v3/ben_ak/gee_outputs'
...
def authenticateGDrive():
    ...
    gauth = ()
    gauth.LoadCredentialsFile("emapr.txt")
    ...
    gauth.SaveCredentialsFile("emapr.txt")
...
def getStatus(startTime, tasks=None, taskFilter='*', columns=None):
    ...
    return taskStatus.loc[~taskStatus.state.isin(['FAILED', 'CANCELED'])]
...
def _callTranslateFromQueue(inQueue):#, out_queue):
   ...
   # while True:
    ...
    if cmd is None:
        ...
        #break
    if len(cmd) == 2:
        callTranslate(cmd, silent=True)
    else:
        callTranslate(cmd, silent=False)
...
def listenAndDownladTasks(tasks, downloadDir, gDriveFolder, gDrive=None, outDir=None, clipFile=None, sleep=.5, njobs=10, silent=True, timestamp_offset=30, logFile=None):
    ...
    while not taskStatus.downloadDone.all():
        ...
        if outDir is not None: # If it's None, don't decompose
            ...
            #Send termination signal to queue
            if taskStatus.downloadDone.all(): 
        ...
        # If outDir is None, 
            ...
            for i, taskName in completeImgs.description.iteritems():
                try:
                    # for some reason this occasionally throws "internal error"
                    matches = listDriveFiles(gDrive, gDriveFolder, downloadDir, searchPatterns=['%s*.tif' % taskName], sizeDifTolerance=500)
                except:
                    continue
                if len(matches) == 0:
                    taskStatus.loc[i, 'downloadDone'] = True
                    
            # check if all downloads are done for sure
            try:
                listedFiles = listDriveFiles(gDrive, gDriveFolder, downloadDir, sizeDifTolerance=500)
            except:
                continue
            if len(listedFiles) == 0 and taskStatus.downloading.all():
    ...
    # Close all processes
    #   wait to for queue to empty because downloads might fail and placed back in queue.
    ...
    if outDir:
        for _ in range(njobs): 
            downloadQueue.put(None) # send termination signal to all processes
    pool.close()
    pool.join()
...
def listDriveFiles(gDrive, gDriveFolder, outDirPath=None, searchPatterns=None, sizeDifTolerance=0):
    ...
    driveDir = gDrive.ListFile({'q': "mimeType='application/vnd.google-apps.folder' and title contains '"+gDriveFolder+"'"}).GetList()
    query = "'%s' in parents" % driveDir[0]['id']
...
def getGdriveFiles(gDriveFolder, outDirPath, njobs=4, gDrive=None, searchPatterns=None):
    t0 = time.time()
    njobs = int(njobs)
    ...
    gDir = gDrive.ListFile({'q': "mimeType='application/vnd.google-apps.folder' and title contains '%s' and trashed=false" % gDriveFolder}).GetList()
    import pdb; pdb.set_trace()
    if len(gDir) == 1:
        ...
        # List files in gDrive folder        
        files = listDriveFiles(gDrive, gDriveFolder, outDirPath, searchPatterns, sizeDifTolerance=500)
        
        if len(files) == 0:
            sys.exit('No incomplete or un-downloaded files found in %s' % gDriveFolder)
        ...
        while nRemaining > 0:
            ...
            if njobs > 1:
                ...
                errors = pool.map(func, files.iterrows(), chunksize=1)
                pool.close()
                pool.join()
            else:
                errors = []
                for f in files.iterrows(): 
                    #returned = download_file(f, outDirPath, t1, nRemaining)
                    import pdb; pdb.set_trace()
                    returned = download_file(gDrive=gDrive, outDirPath=outDirPath, startTime=t1, nFiles=nRemaining)
                    if returned is not None:
                        errors.append(returned)
            
            # Check if any files didn't complete
            #files = getIncompleteFiles(files, outDirPath)
            remaining = [e for e in errors if e is not None]
            files = files[files.title.isin(remaining)]
            nRemaining = len(files)
        ...
        found_folders = '\n\t\t'.join([f['title'] for f in gDir])
        raise RuntimeError('More than one folder found containing the name "%s":\n\t\t%s' % (gDriveFolder, found_folders))
    else:
        raise RuntimeError('Could not find %s' % gDriveFolder)   
...
def getDecomposeCommands(chunkDir, runInfoTxt, bandInfoTxt, outDir, clipFile, tileIdField='name', proj='EPSG:3338', nTiles=9, count=None, nTasks=1, nCommands=None, startTime=None, overwrite=False):
    ...
    mtime = datetime.fromtimestamp(os.stat(runInfoTxt).st_mtime)
    runInfo['dateRange'] = (runInfo.startDay + runInfo.endDay).replace('-', '')
    runInfo['segIndex'] = runInfo.segIndex.lower()
    runInfo['processingDate'] = mtime.strftime('%Y%m%d')
    ...
    filterString = "{featureKey} = {featureValue}".format(**runInfo) #expects integer
    return_code = layer.SetAttributeFilter(filterString)
    if return_code: #0 means success, anything else is an error
        # Try to set the filter as a string
        layer.SetAttributeFilter("{featureKey} = '{featureValue}'".format(**runInfo))    
    ...
    while feature: 
        ...
        print ('tileID is')
        print (tileID)
        #if tileID not in ['0639', '0718', '0797', '0876', '0955', '1034']:
        #    feature = layer.GetNextFeature()
        #    continue
        # 
        print ('indexDir is')
        ...
        for outType in outImgTypes:
            ...
            exists = len(glob.glob(outFile.replace(info['processingDate'], '*'))) > 0
...
def clipAndDecompose(chunkDir, outDir, clipFile, searchStr='', njobs=1, tileIdField='name', proj='EPSG:3338', returnOutDirs=False, overwrite=False):
